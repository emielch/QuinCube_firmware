...
void demoLoop()
{
  ...
  for (uint8_t j = 0; j < 36; j++) {
    float spread = min(max(0, touchVals[j] - 1), 20);
    float position = touchLedLen - touchsegLen * j * 0.75 - touchLedLen / 2 - touchsegLen / 2;
    if (j > 23) position += touchLedLen;
    ...
    for (int i = startLed; i <= endLed; i++) {
      if ( i >= 0 && i < touchLedLen ) {
        ...
        Color col(10 * j, 100, 100, HSB_MODE);
